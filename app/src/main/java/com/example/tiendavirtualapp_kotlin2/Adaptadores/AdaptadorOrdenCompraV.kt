...
import com.example.tiendavirtualapp_kotlin2.Vendedor.Orden.DetalleOrdenVActivity
import com.example.tiendavirtualapp_kotlin2.databinding.ItemOrdenCompraBinding
...
class AdaptadorOrdenCompraV : RecyclerView.Adapter<AdaptadorOrdenCompraV.HolderOrdenCompra>{
    ...
    override fun onBindViewHolder(holder: HolderOrdenCompra, position: Int) {
        ...
        val fecha = try {
            if (tiempoOrden.isNotEmpty() && tiempoOrden != "null") {
                Constantes().obtenerFecha(tiempoOrden.toLong())
            } else {
                "Fecha no disponible"
            }
        } catch (e: Exception) {
